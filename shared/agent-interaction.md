# Coding Agent Interaction Contract

Read this reference immediately before asking the user for input.

1. Follow the active Coding Agent mode. When a blocking user-input tool is available and permitted, use it for a blocking decision. Otherwise ask clear numbered choices in chat, end the turn, and wait for the reply.
...
Skill-specific rules decide what to ask and which defaults are safe. This contract decides how the question reaches the user in Coding Agent.
