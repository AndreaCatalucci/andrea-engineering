...
Invoke `ae-coordinate` explicitly for a large delivery. It turns user-defined scope and available Implementation Plans into a Delivery Map under `docs/delivery-maps/`. The coordinator advances through reconcile, ready, approval, waiting, blocked, and complete states. For each ready packet it chooses whether to reuse a chat or start fresh, explaining why. Focused continuation benefits from retained context; a changed goal or a rabbit hole can warrant a fresh chat with verified facts, failed attempts, and a bounded next question. Ready work receives a complete copyable prompt and a preview of the destination, checkout, code state, model, and reasoning effort. Creation or follow-up delivery waits for explicit approval. Returned reports are verified before dependent work advances.
