...
| PR URL (e.g., `https://HOST/OWNER/REPO/pull/123`, no comment fragment) | **Full** -- all unresolved threads on that PR; parse `HOST`, `OWNER/REPO`, and the number from the URL (this is how `ae-babysit-pr` hands a fork→upstream PR to full mode against the right host/base) |
