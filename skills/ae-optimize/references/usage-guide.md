# `$ae-optimize` Usage Guide

## What This Skill Is For

`$ae-optimize` is for hard engineering problems where:
...
Use `$ae-optimize` when the problem looks like:
...
`$ae-optimize` is usually the wrong tool when:
...
```text
Use $ae-optimize to find the smallest memory setting that keeps this service stable under our load test.
...
```text
Use $ae-optimize to improve issue and PR clustering quality.
...
```text
Use $ae-optimize to create a summarization prompt for issues and PRs that minimizes token spend while still producing summaries that are good enough for downstream clustering.
