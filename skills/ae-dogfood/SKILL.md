---
name: ae-dogfood
description: "Run hands-off, diff-scoped browser QA with autonomous fixes and a durable report. Use when asked to dogfood or QA a branch or PR in a real browser; not for narrow scripted browser checks."
---
...
## Use the Codex Browser

This workflow drives the browser through Codex's in-app Browser. Load and follow the `browser:control-in-app-browser` skill before browser actions. Use the Browser's visible page state, screenshots, and console evidence; do not substitute an unrelated browser CLI.

## Prerequisites

- A local dev server you can start (`bin/dev`, `rails server`, `npm run dev`, etc.).
- The `browser:control-in-app-browser` skill is available. If it is unavailable, stop and report that this workflow requires the Codex in-app Browser.

## Reusing Compound-Engineering Skills

`ae-dogfood` is an orchestrator. Prefer delegating to existing CE skills over re-deriving their behavior:

| When | Skill | Why |
|------|-------|-----|
| Phase 0 isolation | `ae-worktree` | Run the dogfood in an isolated worktree so the main checkout stays clean. |
| A failure's root cause is non-obvious | `ae-debug` | Systematic root-cause analysis instead of guess-and-check. |
| Committing each fix | `ae-commit` | Consistent, well-scoped commit messages. |
| A bug reveals a reusable lesson | `ae-compound` | Capture the learning so the team compounds knowledge. |
...
3. Serve        Detect port, start dev server, open it in the Codex Browser
4. Execute      Work the matrix one item at a time in the Codex Browser
...
2. **Refuse to run on the trunk — branch/blank targets only.** If a *branch-name or blank* target resolves to the trunk (`main`/`master`/the detected default), stop — there is no diff to dogfood. A **PR is always diffable** (it has a base), so this check never applies to a PR target; never refuse `$ae-dogfood <number>` just because the PR's head branch happens to be named `main`.
3. **Decide isolation by what you're testing; let `ae-worktree` own the worktree mechanics.** Do not re-derive worktree detection or creation here — `ae-worktree` handles existing Codex isolation, attaching to a ref, and the "already checked out" constraint, and reports its decision back. The only call this skill makes is *whether to ask for isolation at all*:
   ...
   - **A PR or a different named branch:** this is an existing ref to test without disturbing your current checkout. Read and follow [`references/codex-interaction.md`](references/codex-interaction.md), then offer isolation. On **yes**, invoke `ae-worktree` to isolate **that target ref** — it attaches a worktree to the ref (or, if already isolated, checks it out in place; or reports "already checked out at `<path>` — work there" when the ref is live elsewhere). Act on `ae-worktree`'s verdict; the primary checkout is never switched. On **no**, check the target out in place (`gh pr checkout <number>` for a PR, `git checkout <branch>` for a branch), confirming first if uncommitted changes would be disturbed.
...
- **The Codex plan** (`update_plan`) is the live to-do — one task per matrix scenario. Mark each `in_progress` when you start it and `completed` only when it genuinely passes.
...
**Load the matrix into `update_plan`**, one task per scenario, so progress is tracked and nothing is skipped. Order tasks by flow, following the flowcharts, not by file.
...
Open `http://localhost:${PORT}` in the Codex in-app Browser and inspect the rendered page before starting the matrix.
...
2. **Drive it** with the Codex Browser — navigate, inspect visible and interactive state, click, fill, submit, and follow the journey to its real end state. Capture a screenshot when visual evidence materially helps and inspect console/network failures exposed by the Browser. Keep transient screenshots out of the repository; only save one beside the report when it will be embedded in that report.
...
1. Investigate the root cause. If it's non-obvious, use `ae-debug`.
...
4. Commit the fix with a clear message (use `ae-commit`). One logical fix per commit.
5. Re-run the failing scenario in the browser to confirm it now passes; then continue the matrix.
6. If the bug carried a reusable lesson, capture it with `ae-compound`.
