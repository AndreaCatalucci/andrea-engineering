...
- **New work (default).** No specific ref named — create a fresh branch from a base (trunk). This is what `ae-work` uses.
...
`ae-work` and `ae-code-review` offer this skill as an option. When the user selects "worktree" in those flows, run Step 0 first: if the work is already isolated, proceed in place; otherwise create one with plain Git and a meaningful branch name derived from the work description.
