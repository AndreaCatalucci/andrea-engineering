...
```text
.context/andrea-engineering/ce-optimize/<spec-name>/
```
...
Offer a next step only after the verified result is durable: another bounded round, handoff to `ce-work`, or shipping through the user's normal workflow. Do not commit, push, or open a PR unless the user or invoking workflow authorized it.
