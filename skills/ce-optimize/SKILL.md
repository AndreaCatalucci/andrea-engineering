...
The files under `.context/andrea-engineering/ce-optimize/<spec-name>/` are local scratch state. They are ignored by git, so they survive local resumes on the same machine but are not preserved by commits, branches, or pushes unless the user exports them separately.
...
### File Locations (all under `.context/andrea-engineering/ce-optimize/<spec-name>/`)
...
6. Write the spec to `.context/andrea-engineering/ce-optimize/<spec-name>/spec.yaml`
...
**If branch exists**, check for an existing experiment log at `.context/andrea-engineering/ce-optimize/<spec-name>/experiment-log.yaml`.
...
```bash
mkdir -p .context/andrea-engineering/ce-optimize/<spec-name>/
```
...
1. Create the experiment log file at `.context/andrea-engineering/ce-optimize/<spec-name>/experiment-log.yaml`
...
4. **Write strategy digest** to `.context/andrea-engineering/ce-optimize/<spec-name>/strategy-digest.md`:
...
rm -f .context/andrea-engineering/ce-optimize/<spec-name>/strategy-digest.md
```
