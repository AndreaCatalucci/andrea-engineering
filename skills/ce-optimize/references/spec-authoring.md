...
Write the result to `.context/andrea-engineering/ce-optimize/<spec-name>/spec.yaml`, validate every rule in `optimize-spec-schema.yaml`, present it for approval, and stop at the approval gate. This branch is complete only when the saved spec validates and the user has approved it.
