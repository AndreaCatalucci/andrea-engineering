...
1. Search first with `rg --files` and `rg` across manifests, lockfiles, and relevant modules, then read targeted ranges. Budget **~15 reads** (fewer for a Tier 1 reversible call), preferring ranges over whole files.
