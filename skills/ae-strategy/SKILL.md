...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `ae-brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
- Does not write product requirements or implementation plans - those are `ae-brainstorm` and `ae-plan`.
