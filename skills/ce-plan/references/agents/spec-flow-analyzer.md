...
1. Use `rg` to find code related to the feature area—models, controllers, services, routes, and tests
2. Use `rg --files` to find related features that may share patterns or integrations
...
Use `rg --files` for file discovery and `rg -li` for content search, preserving Phase 1's patterns and case-insensitivity.
