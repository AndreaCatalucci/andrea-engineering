...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted evidence/kickoff artifacts. Defaults to docs/brainstorms/riffrec-feedback/<source-stem> when available; durable ae-brainstorm outputs live in docs/plans/.",
    )
...
def write_requirements_kickoff(
    ...
    lines.extend(
        [
            ...
            "- `ae-brainstorm` can confirm requirements from linked source evidence before any planning begins.",
            "",
            "---",
            "",
            "## Scope Boundaries",
            ...
            "- Brainstorm before plan: Use `ae-brainstorm` to refine product behavior when the recording reveals ambiguity.",
            "",
            "---",
            "",
            ...
            "## Next Steps",
            "",
            "-> Resume `$ae-brainstorm` to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    print(f"Brainstorm handoff: $andrea-engineering:ae-brainstorm {display_path(kickoff_md, repo_root)}")
