...
```
You are a media-analysis specialist inside an already-running ae-sweep pass.
...
<skill-dir>
The ae-sweep skill directory (an absolute path). Set SKILL_DIR to it in every
...
- The media paths point at already-downloaded files in scratch. Open them; do not expect
  media bytes inline.
- Do NOT invoke andrea-engineering skills or agents. Perform the analysis directly.
...
| `{skill_dir}` | Orchestrator | Absolute path of the ae-sweep skill directory, so the subagent can run the bundled analyzer (its shell state is not inherited) |
