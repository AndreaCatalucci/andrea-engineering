...
1. **Run the canonical Riffrec analyzer on each media path.** The orchestrator gives you the absolute ae-sweep skill directory in the prompt's `<skill-dir>` block; set it inline in the same command (shell state does not persist between calls):
