...
Create `/tmp/andrea-engineering/ae-code-review/<run-id>/`. Stage large diffs/file lists there once and pass paths instead of duplicating contents.
...
Always write run artifacts under `/tmp/andrea-engineering/ae-code-review/<run-id>/`, including synthesized findings, actionable findings, advisory outputs, per-agent JSON, metadata, and `report.md` or `review.json`.
