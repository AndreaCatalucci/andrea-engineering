...
This is the canonical section skeleton. Shape each finding for the reader's next action per SKILL.md “Present and persist”: what/where, why it matters, required response, and confidence. Group findings by severity, not reviewer.
...
- Removable code: ~40 lines / 1 file across findings #5 (warning only, not a target)
...
- **ASCII-safe only** -- never box-drawing characters or per-item horizontal-rule separators (`────`) between entries (the report-level `---` before the verdict is still required), no Unicode arrows or middot; use `->`. Tables are a good default but not mandatory -- let the shape serve the finding type; stay consistent within a section
...
- **Triage Groups section (when groups exist)** -- pipe table `| Group | Findings | Context | Preferred Resolution | Why |` rendered after Applied and before severity tables. Every referenced stable `#` must survive validation/apply. Groups supplement findings; they never replace or renumber them.
- **Applied section (default mode only)** -- when fixes were applied, list them first as `# | File | Fix | Reviewer`, followed by validation and commit status. A multi-file fix is one row/stable number. Flag auth/contract/concurrency edits for human diff attention. Omit in `mode:agent` and when empty.
...
- **Learnings & Past Solutions section** -- strong inline matches from `docs/solutions/`, with links
...
- **Coverage section** -- suppressed count, removable code (only when deletion-oriented maintainability findings exist; approximate net lines/files removable if applied -- a dead-weight warning, never a reduction target, omit otherwise), residual risks, testing gaps, failed reviewers
...
- **No `applied_fixes` and no Applied section** — `mode:agent` does not apply fixes; the handoff is `actionable_findings`.
...
- **Stable `#`** — same numbering as merge synthesis, carried in JSON for downstream apply/residual tracking.
