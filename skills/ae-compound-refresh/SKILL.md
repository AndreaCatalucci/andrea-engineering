...
If no candidates exist, report that `ae-compound` can create the first learning after a verified problem is solved.
...
`ae-compound` captures a newly solved problem. `ae-compound-refresh` maintains the accuracy and shape of the existing knowledge set.
