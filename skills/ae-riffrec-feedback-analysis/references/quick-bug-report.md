...
1. Run the analyzer to a temp directory so nothing pollutes the repo (`SKILL_DIR` is the directory containing the `ae-riffrec-feedback-analysis` SKILL.md; set it in the same command because Codex shell calls do not share shell state):
...
- **Suggested next step** — single sentence: file an issue, open `ae-debug`, or escalate to extensive analysis if more issues surfaced.
...
- No automatic handoff to `ae-brainstorm`. The quick path ends with the bug report.
