---
name: ae-riffrec-feedback-analysis
...
All non-setup paths share the same analyzer, which ships in this skill's `scripts/` directory. Codex shell calls run from the user's project, not the skill directory, so a bare `scripts/<name>` path will not resolve. Invoke it by the skill's own absolute path: set `SKILL_DIR` to the directory you loaded this `ae-riffrec-feedback-analysis` SKILL.md from in the same command, because shell state does not persist between calls:
