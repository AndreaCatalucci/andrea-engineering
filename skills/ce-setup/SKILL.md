...
if [ -f "$SKILL_DIR/scripts/check-health" ]; then bash "$SKILL_DIR/scripts/check-health"; else echo "Bundled health script not found at $SKILL_DIR/scripts/check-health; run the inline checks from ce-setup instead."; fi
```
...
Report repo-local fixes applied, fixes declined, and missing optional tools or that all are available. Mention that `ce-setup` can be rerun anytime.
