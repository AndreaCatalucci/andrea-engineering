...
Detect the installed andrea-engineering plugin version by reading the plugin metadata or manifest when the platform exposes it. If the version cannot be determined, skip this step.
...
```text
Andrea Engineering -- checking your environment...
```
...
```text
✅ Andrea Engineering setup complete
...
If `andrea-engineering.local.md` exists at the repo root, explain that it is obsolete because review-agent selection is automatic and surviving machine-local settings now live in `.andrea-engineering/config.local.yaml`.

Ask whether to delete it now. Delete only if the user approves.

### Step 5: Refresh Example Config

Copy `references/config-template.yaml` to `<repo-root>/.andrea-engineering/config.local.example.yaml`, creating the directory if needed. This file is committed to the repo and should always reflect the latest available settings.
...
If `.andrea-engineering/config.local.yaml` does not exist, ask:

```text
Set up a local config file for this project?
This saves optional Andrea Engineering preferences such as output formats and product pulse settings.
...
If the user approves, copy `references/config-template.yaml` to `<repo-root>/.andrea-engineering/config.local.yaml`.

### Step 7: Ensure Local Config Is Gitignored

If `.andrea-engineering/config.local.yaml` exists and is not covered by `.gitignore`, offer to add:

```text
.andrea-engineering/*.local.yaml
```
