...
It is one simple choice: do they want a `$ce-pov` verdict, or not? Ask through the [shared codex-interaction contract](codex-interaction.md):
...
Name `$ce-pov` by what it does for the user (it gives a project-grounded verdict on the candidate), never as internal machinery.
