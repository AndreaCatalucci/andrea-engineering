Derive the **question-agnostic project profile** for the repository at the current working directory — the stable orientation that repo-grounding skills reuse. The calling agent follows this contract inline on a cache miss, writes the output to the shared profile cache, and reuses it across skills and sessions at this commit.
