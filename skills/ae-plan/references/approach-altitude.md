...
This is not a software plan template (no work steps / test scenarios) unless the deliverable itself is a software implementation plan — in which case "execute now / code" routes into the normal `ae-plan` flow (below) rather than composing the deliverable here.
...
3. Return the saved plan path. Run `ae-work` only after a later explicit user request.

`ae-plan` itself does not execute the deliverable in any path — it produces the approach-plan and hands off. The portable plan is also runnable directly by Codex without `ae-work`.
