...
This content is loaded when a synthesis-summary phase fires in ae-plan. There are two variants — they share structure but differ in timing and content focus:
...
1. **Brainstorm-scope restatement** (1-2 sentences, prose). Restates the brainstorm's scope as orientation. The user wrote this content, but the synthesis may be read days later or in parallel with other plans — the restatement is the topic anchor that says "this is the plan we're planning against." Stay in the brainstorm's own vocabulary. Do NOT enumerate Work Steps, restate constraints back at the user, or list acceptance examples.
...
- AND Phase 0.4 stayed in ae-plan (did not route to ae-debug, ae-work, or universal-planning)
...
Confirm and I'll proceed to research, drawing on this scope. (You can also redirect to $ae-brainstorm if this is bigger than you initially thought — I'll stop here and load it for you.)
```
...
from `ae-brainstorm` stores its confirmed summary, problem, requirements, flows,
...
When the soft-cut fires, ask through the [shared codex-interaction contract](codex-interaction.md) with two options:
...
When the skill is invoked from an automated workflow such as LFG or another headless context, it runs in non-interactive mode. The artifact is read by downstream skills (`ae-doc-review`, `ae-work`) and human reviewers.
...
This restores the audit visibility the original design intended (un-validated bets must not propagate as authoritative content), but surfaces them under their own label rather than hiding them. Downstream review (ae-doc-review, ae-work, human PR review) can scrutinize Assumptions specifically.
...
- **Solo variant**: common redirects include "this is bigger than I thought — let me brainstorm first" (suggest `$ae-brainstorm`), "this is just a fix, no plan needed" (suggest `$ae-work`), or "I need to investigate first" (suggest `$ae-debug`).
- **Brainstorm-sourced variant**: less common, but possible — "actually this scope is wrong, take it back to brainstorm" (suggest `$ae-brainstorm` to revise the upstream doc).

In either case: stop ae-plan, suggest the alternative skill, offer to load it in-session. Don't push back or argue — the user's redirect signal is the deliberate choice.
