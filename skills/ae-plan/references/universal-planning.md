...
This file is loaded when ae-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from LFG or another headless workflow: output "This is a non-software task. The LFG pipeline requires ae-work, which only supports software tasks. Use `$ae-plan` directly for non-software planning." and stop.
- **Software-plan guard.** Universal-planning outputs are not software
  implementation plans. Do not label them `plan_format: andrea-plan/v1` or
  produce a `/goal` launch block unless they contain What We're Building, How
  We'll Build It, Work Steps, How We'll Check It, and Done When.
...
**Execution here is research and analysis only — never code.** Reading code and artifacts to understand them is in-bounds research; writing or running code to change the system is not — that belongs in `ae-work`. This keeps the planning/execution boundary intact.
...
> Wrong: "Quick note first: $ae-plan builds implementation plans, so I ignored the template and just answered the question. Here's what the data says..."
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `$ae-plan` because they want a good plan, not a disclaimer about training data.
...
Create a structured plan guided by these quality principles. Do NOT use the software plan template (work steps, test scenarios, file paths, etc.).
