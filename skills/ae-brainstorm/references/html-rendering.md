...
so the ae-plan handoff gates the 5.3.8 doc-review pass to `OUTPUT_FORMAT=md`
...
2. `docs/DESIGN.md`.
3. `.andrea-engineering/DESIGN.md`.
...
`ae-brainstorm` writes (now under `docs/plans/`), not an implementation-ready
plan (`ae-plan`'s enriched output) — and only to UI-shaped requirements — a
