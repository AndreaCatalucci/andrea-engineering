...
New `ae-brainstorm` outputs live under `docs/plans/` and use the plan
plan format:
...
look executable and waste downstream tokens. `ae-plan` adds those sections when
...
- **Success Measures** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("ae-doc-review can act on this
...
- **`plan_readiness`** — always `requirements-only` for new
  ...
  or `done`.
- **`requirements_source`** — always `ae-brainstorm`.
