...
`ce-ideate` answers: “What are the strongest ideas worth exploring?” It writes
a ranked ideation document. It does not write requirements, plans, or code.
Use `ce-brainstorm` when the product meaning or scope is unclear and `ce-plan`
...
`/tmp/andrea-engineering/ce-ideate/<run-id>`. Reuse it for caches, candidate
